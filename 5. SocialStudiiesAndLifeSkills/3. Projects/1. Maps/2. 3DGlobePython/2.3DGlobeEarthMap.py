...
def download_and_extract(url, extract_path="natural_earth_data"):
    """Downloads and extracts a shapefile."""
    try:
...
def create_globe():
    ...
    # --- Load Map Data ---
    shapefile_url = "https://www.naturalearthdata.com/downloads/110m-cultural-vectors/110m-admin-0-countries/"
    ...
    if not os.path.exists(shapefile_path):
        try:
            headers = {'User-Agent': 'Mozilla/5.0'}
            response = requests.get(shapefile_url, headers=headers)
            response.raise_for_status()
            soup = BeautifulSoup(response.text, 'html.parser')
            download_link = requests.compat.urljoin(shapefile_url, soup.find('a', href=lambda h: h and "ne_110m_admin_0_countries.zip" in h)['href'])
            if not download_and_extract(download_link, extract_path): return
    ...
    def on_key(event):
        ...
        if event.key in ('left', 'right', 'up', 'down'):
            ...
            if event.key == 'shift+right':
                current_azim -= 5  # Rotate clockwise
            elif event.key == 'shift+left':
                current_azim += 5  # Rotate counter-clockwise
            elif event.key == 'shift+up':
                current_elev += 5 # Rotate forward
            elif event.key == 'shift+down':
                current_elev -= 5 # Rotate backward
    ...
    fig.text(0.5, 0.95, "Use arrow keys to move the marker; Shift+arrow keys to rotate.", ha='center', fontsize=14)

    update_marker()  # Initial marker placement
    plt.show()
